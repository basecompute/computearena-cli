...
| Runtime invocation | Requested PP sweep forwarded, separate PP/TG processes, llama.cpp PP depth zero and TG depth one (`-gp 1,128` for an ik_llama.cpp build), JSON output, options after `--` forwarded with workload options refused, no replay telemetry flag for current BaseRT, no default cooldown, native warmup disablement |
